...
public class PlayFabDataManager : IDataManager {
	...
	public  void GetPlayerScore (Action<int> Callback) {
		GetPlayerStatisticsRequest request = new GetPlayerStatisticsRequest () {
			StatisticNames = new List<string> () { "Level_" + LevelManager.Instance.currentLevel }
		};
		...
		PlayFabClientAPI.GetPlayerStatistics (request, (result) => {
			if ((result.Statistics == null)) {
				...
				foreach (var item in result.Statistics) {
					if (item.StatisticName == "Level_" + LevelManager.Instance.currentLevel) {
						Callback (item.Value);
					}
...
//		for (int i = 1; i <= LevelManager.Instance.currentLevel; i++) {//1.3 
