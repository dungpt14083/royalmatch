namespace Water2D
{
    ...
    public class Water2D_Spawner : MonoBehaviour
    {
        ...
        public void SetupParticles()
        {
            ...
            {
                WaterDropsObjects[i] =
                    Instantiate(DropObject, gameObject.transform.position, new Quaternion(0, 0, 0, 0)) as GameObject;
        ...
        void loop_editor(Vector3 _pos, Vector2 _initSpeed, int count = -1, float delay = 0f,
            bool waitBetweenDropSpawn = true)
        {
        ...
        IEnumerator loop(Vector3 _pos, Vector2 _initSpeed, int count = -1, float delay = 0f,
            bool waitBetweenDropSpawn = true)
        {
            if (IsSpawning)
                yield break;
            ...
            {
                Debug.LogError(
                    "You're trying spawn particles in a Filler type. You should create a water spawner instead");
                yield break;
            }
        ...
        IEnumerator CheckOnFill(Collider2D shapeCollider, float accuracy = .8f)
        {
            ...
            while (true)
            {
                ...
                if (_trigged)
                {
                    ...
                    Debug.Log("Fill Event sucessful Complete! : droplives:" + DropsUsed + "   // " +
                              ((int)(DropsUsed * accuracy)).ToString() + "within the target");
