...
public class GoalSpawner : MonoBehaviour
{
    ...
    GameObject coinPre;
    public void SpawnCoin(Transform parent)
    {
        int _currentTotal = 0;
        int _random = 0;
        ...
        while(_currentTotal < totalCoin)
        {
            ...
            if (_random >= 0 && _random < 3)
                coinPre =  Instantiate(coin1, pos, transform.rotation, parent);
            else if(_random >= 3 && _random < 6)
                coinPre = Instantiate(coin2, pos, transform.rotation, parent);
            else if (_random >= 6 && _random < 8)
                coinPre = Instantiate(coin4, pos, transform.rotation, parent);
            else
                coinPre = Instantiate(coin3, pos, transform.rotation, parent);
            _currentTotal++;
        }
    }
}
